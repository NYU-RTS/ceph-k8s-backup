This is an application used to make regular backups of Ceph data in RBD volumes in a Kubernetes cluster.

It runs a container that periodically enumerates PersistentVolumeClaims on the cluster and backs them up using Restic.

# What's backed up

It supports RBD filesystems and RBD block devices:
...
* CephFS volumes are distributed file shares that are accessed using a file-based API. Their advantage is that they can be mounted on multiple machines at the same time, and Ceph can apply access control to directories.
    * Users can't delete the filesystem, and you can attach it multiple places without special magic
    * Just do regular snapshots of the CephFS, run Restic on the snapshot if you want to
    * This operator doesn't do anything with CephFS volumes as I have not needed it

# Where is it backed up

The data backed up with Restic ends up in Restic repositories, one per namespace. Each PersistentVolumeClaim appears as a different "host" in Restic:
...
* How often to run
* The Restic repositories' base URL
...
In addition, an annotation `cephbackup.hpc.nyu.edu/last-backup` is set on the PVC by this system to show the date of the last backup.
